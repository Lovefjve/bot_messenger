  ...
  return async function ({ event }) {
    ...
    if (!(senderID in usages)) {
      ...
      if (ADMINBOT.includes(senderID)) {
        usages[senderID].usages = -1; // Set usages to -1 for adminbot
      } else {
    ...
    if (!global.data.allThreadID.includes(threadID) && !ADMINBOT.includes(senderID) && adminbot.adminPaseOnly == true) {
      return api.sendMessage("Chỉ Admin mới được sử dụng bot trong chat riêng.", threadID, messageID);
    }
    if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {
      if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) return api.sendMessage('Chỉ ADMIN mới có thể sử dụng bot', threadID, messageID);
    }
    if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) {
      return api.sendMessage('Chỉ SUPPORT mới có thể sử dụng bot', threadID, messageID);
    }
    const dataAdbox = require('./../../modules/commands/cache/data.json');
    var threadInf = threadInfo.get(threadID) || await Threads.getInfo(threadID);
    // Kiểm tra threadInf và adminIDs trước khi sử dụng
    if (threadInf && threadInf.adminIDs) {
      ...
      if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true && !NDH.includes(senderID)) {
        return api.sendMessage('Hiện tại đang kích hoạt chế độ Quản trị viên, chỉ Quản trị viên được sử dụng Bot', event.threadID, event.messageID);
      }
    } else {
      // Xử lý trường hợp không thể lấy được thông tin thread hoặc adminIDs không tồn tại
      logger("Không thể lấy thông tin thread hoặc adminIDs không tồn tại.");
      return api.sendMessage('Lỗi: Không thể lấy thông tin thread hoặc adminIDs không tồn tại.', threadID, messageID);
    }
    ...
    if (!ADMINBOT.includes(senderID) && usages[senderID].usages <= 0 && !["nhanluot", "check", "luotdung", "daily"].includes(commandName)) return api.sendMessage("Bạn hiện đã hết lượt sử dụng Bot\nDùng lệnh 𝒅𝒂𝒊𝒍𝒚 để nhận thêm lượt dùng\nDùng lệnh 𝒍𝒖𝒐𝒕𝒅𝒖𝒏𝒈 để xem lượt dùng Bot", threadID, messageID);
    if (!command) {
      var allCommandName = [];
    ...
    try {
      ...
      if (!ADMINBOT.includes(senderID) && !["nhanluot", "check", "luotdung", "daily", "menu"].includes(commandName)) usages[senderID].usages -= 1;
      fs.writeFileSync(usgPath, JSON.stringify(usages, null, 4));
      command.run(Obj);
