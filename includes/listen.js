...
(async function () {
    ...
    try {
      ...
      for (const dataU of users) {
    	...
    	logger.loader(`BOT BẮT ĐẦU NHẬN LỆNH`);
    } catch (error) {
        return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
    }
}());
  logger(`BOT PRO MAX`, "TatsuYTB");
  logger(`${global.config.ADMINBOT} - JRT`, "ADMINBOT INFO");
  logger(`${api.getCurrentUserID()} - » ${global.config.PREFIX} « • ${(!global.config.BOTNAME) ? "TatsuYTB" : global.config.BOTNAME}`, "BOT INFO");
  ...
  return async (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID) &&!ndhBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`Đã gửi yêu cầu đến Admin Bot`, event.threadID, () => {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Nhóm của bạn chưa được duyệt sử dụng nếu bạn là admin bot hãy ghi\n${prefix}duyetbox để có thể sử dụng`, event.threadID);
