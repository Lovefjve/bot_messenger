...
module.exports.run = async function({ api, event, args }) {
	...
	return api.sendMessage(`[😽]➜ Uid của bạn đây: ${uid}`, event.threadID, event.messageID) }
    ...
    return api.sendMessage(`${res_ID}`, event.threadID, event.messageID) }
		...
		for (var i = 0; i < Object.keys(event.mentions).length; i++) api.sendMessage(`[😽]➜ Tên: ${Object.values(event.mentions)[i].replace('@', '')}\n[😽]➜ UID: ${Object.keys(event.mentions)[i]}`, event.threadID);
		return;
	}
}
}