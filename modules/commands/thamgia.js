    ...
    if (!threadList[parseInt(body) - 1]) return api.sendMessage("➜ Lựa chọn của bạn không nằm trong danh sách", threadID, messageID);
        ...
        try {
            ...
            api.addUserToGroup(senderID, threadInfo.threadID, (e) => {
              ...
              else api.sendMessage(`➜ Bot đã thêm bạn vào nhóm ${threadInfo.name} rồi nha. Kiểm tra ở mục spam hoặc tin nhắn chờ nếu không thấy box nka.`, threadID, messageID);
            });
        }
        catch (error) {
            return api.sendMessage(`➜ Bị lỗi: ${error}`, threadID, messageID);
        }
    }
};
