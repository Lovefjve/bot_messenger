...
if (args[0] == "info") {
  ...
  for (let i = 1; i < 8; i++) {
    ...
    msg += `${i == 7 ? "Chủ Nhật" : "Thứ " + (i+1)}: 💸 ${addMoney} $ tiền mặt\n\n`;
  }
...
if (args[0] == "7day") {
  if (dataUser[senderID].diemdanh <= 6) {
    return api.sendMessage(
      `→ Bạn mới điểm danh được ${dataUser[senderID].diemdanh} ngày thôi!!, đủ 7 ngày liên tục mới nhận được phần quà bí mật nha`,
      threadID);
  }
...
fs.writeFileSync(path, JSON.stringify(dataUser));
 ...
 + "      + " + usages7Day.toLocaleString() + " Lượt dùng\n"
 + "◆━━━━━•💜•━━━━━◆\n\n"
...
return api.sendMessage({
 ...
 + "      + " + addUsages.toLocaleString() + " Lượt dùng\n"
 + "      + " + 1 + " Điểm đăng nhập\n\n"
 + "→ Bạn hiện tại có " + dataUser[senderID].diemdanh + " Điểm đăng nhập\n"
