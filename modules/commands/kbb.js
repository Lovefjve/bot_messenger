...
module.exports.run = async function({ api, event, args, Users, Currencies }) {
    ...
    var user = args[0];
    var coins = args[1];
    if(!user) return api.sendMessage('[Kéo Búa Bao] vui lòng chọn kéo, búa hoặc bao', threadID, messageID);
    if(listItem.includes(user.toLowerCase()) == false) return api.sendMessage('[Kéo Búa Bao] Lựa chọn không hợp lệ', threadID, messageID);

    var fu = listItem.findIndex(i => i == user);
    var fb = listItem.findIndex(i => i == bot);
    var a = [fu, fb];

    if (args[1] < 5000 || isNaN(args[1])) return api.sendMessage(`[Kéo Búa Bao] Mức đặt cược của bạn không phù hợp hoặc dưới 5000$`, threadID, messageID);
    if (money < coins) return api.sendMessage(`[Kéo Búa Bao] Bạn không đủ ${coins}$ để chơi`, threadID, messageID);
    ...
    var compare = function (choice1, choice2){
        ...
        if(choice1 == 'búa') {
            ...
            if(choice2 == 'bao') {
                checkwin.push(`[𝐊𝐞̂́𝐭 𝐐𝐮] - Thua ${msgLose}`)
                checkwin.push(1)
                return checkwin
            }
        }
